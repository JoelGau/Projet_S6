...
const uint8_t AT30TSE758_TempRead [SLAVE_MEM_ADDR_LENGTH] = {
 ...
 void AT30TSE758_init() {
 	// Sets AT30TSE758
 	const uint8_t command[] = {0x60,0x00}; // 12 bits resolution temperature

	twi_package_t packet_write = {
		.addr[0]      =  AT30TSE758_TempWrite[0],		/* TWI slave memory address data */
		.addr_length  = (uint8_t)SLAVE_MEM_ADDR_LENGTH, /* TWI slave memory address data size */
 ...
 short getTemperature() {
	...
	twi_package_t packet_read = {
		.addr[0]      =  AT30TSE758_TempRead[0],		/* TWI slave memory address data */
		.addr_length  = (uint8_t)SLAVE_MEM_ADDR_LENGTH, /* TWI slave memory address data size */
		.chip         = SLAVE_BUS_ADDR,					/* TWI slave bus address */
